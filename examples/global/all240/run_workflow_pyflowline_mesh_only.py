...
#date time
sDate_today = '20260303'  #use a fixed date for easy repeatability
...
dSpac_value = 240  #unit in km, this is the most important parameter to control the overall resolution of the mesh, which will be used for both river and watershed boundary control
dResolution_ocean = 240
dResolution_land = 240  #unit in km
dResolution_coastline = 240  #unit in km

#for coastline
dThreshold_area_island = 1.0E3 * 1.0E6  #unit m2, this one may need to be adjusted based on the resolution
dResolution_coastline_buffer = dResolution_coastline * 1.0E3  #buffer zone for coastline line
#small island removal threshold

dDrainage_area_threshold= dResolution_land * dResolution_land *10 * 1.0E6  #at least ten grid cells of drainage area, this may be adjusted as well
...
if iFlag_simplify_hydrosheds_river_network == 1:
    nOutlet_largest = simplify_hydrorivers_networks(sFilename_flowline_hydrosheds_in,
    ...
    print('Actual number of outlet processed: ', nOutlet_largest)
else:
...
if iFlag_flexible_river_mouth == 1:
    ...
    if os.path.isfile(sFilename_flowline_outlet_for_tag) is False:
        sFilename_flowline_outlet_for_tag = sFilename_flowline_hydrosheds_out
    tag_river_outlet(sFilename_flowline_outlet_for_tag,
                     sFilename_river_network_raster,
                     sFilename_river_network_raster_mouth_tagged,
                     iSpecial_value_outlet_in=255)
    print('Tagged river mouth into raster file: ', sFilename_river_network_raster_mouth_tagged)

sFilename_river_network_raster = sFilename_river_network_raster_mouth_tagged
...
if iFlag_process_coastline == 1:
    #sFilename_tif_wo_island, sFilename_vector_geojson_merged = create_land_ocean_mask_from_hydrobasin(sWorkspace_coastline_output,
    ...
    for i in range(1, nOutlet_largest+1):
        sBasin_id = '{:04d}'.format(i)
        sFilename_flowline_simplified_basin = os.path.join(sWorkspace_river_network_output, 'HydroRIVERS_v10_simplified_' + sDistance_tolerance + '_' + sDrainage_area_threshold +'_'+ sBasin_id + '.geojson')
        if os.path.isfile(sFilename_flowline_simplified_basin):
            aFilename_flowline.append(sFilename_flowline_simplified_basin)
        else:
            #sometimes not all nOutlet_largest are available due to the drainage area threshold, we just skip those missing ones
            continue
...
if iFlag_debug == 1:
    ...
    change_json_key_value(sFilename_jigsaw_configuration_copy, "dSpac_value", dSpac_value) #set the resolution for coastline line
    ...
    oPyflowline._pyflowline_create_hpc_job(sSlurm_in = 'short', hours_in = 2 )
